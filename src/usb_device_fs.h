// usb_device_fs.h — USBFS (Full-Speed) device-clone backend behind the
// usb_device.h dispatcher.
//
// usbfsd_* names coexist with the USBHSD backend (usb_device_hs.c). Selected at
// runtime for FS/LS clones (captured_descriptors_t.speed == USB_SPEED_FULL or
// USB_SPEED_LOW); the USBHS device controller does not enumerate at Full-Speed,
// so FS devices clone on the separate USBFS controller (PA11/PA12, USB-C port).
...
bool usbfsd_in_ep_free(uint8_t ep_num);
