...
// silent, ACK-paced by usb_device_in_ep_free() and gated by merged_this_cycle so
// the merge and synth paths never both emit in one frame. humanize_inject_emit()
// runs per emitted frame inside usb_merge_take_injection(), not per command, to
// preserve sub-pixel-carry behavior without adding perpendicular noise (noise=0
// quantizer path).
...
int16_t usb_merge_peek_inject_dx_for_test(void) { return inject.mouse_dx; }
int16_t usb_merge_peek_inject_dy_for_test(void) { return inject.mouse_dy; }
int8_t  usb_merge_peek_inject_wheel_for_test(void) { return inject.mouse_wheel; }
                              ...
                              int8_t wheel)
{
	...
	int32_t aw = (int32_t)inject.mouse_wheel + wheel;
	if (aw >  127) aw =  127;
	if (aw < -127) aw = -127;
	inject.mouse_wheel = (int8_t)aw;
	int32_t ax = (int32_t)inject.mouse_dx + dx;
	int32_t ay = (int32_t)inject.mouse_dy + dy;
	if (ax >  INJECT_ACCUM_CAP) ax =  INJECT_ACCUM_CAP;
	if (ax < -INJECT_ACCUM_CAP) ax = -INJECT_ACCUM_CAP;
	if (ay >  INJECT_ACCUM_CAP) ay =  INJECT_ACCUM_CAP;
	if (ay < -INJECT_ACCUM_CAP) ay = -INJECT_ACCUM_CAP;
	inject.mouse_dx = (int16_t)ax;
	inject.mouse_dy = (int16_t)ay;
...
// paced by the device IN-EP-free (ACK) gate, and flushes an unconsumed wheel on a
// separate report ID.
...
void usb_merge_send_pending(void)
{
	...
	// Cadence is ACK-paced, not time-capped: emit only when the device IN EP has
	// drained the previous report (back to NAK). This self-paces to the cloned
	// device's real poll rate — 1 kHz on an FS clone, up to 8 kHz on HS — and
	// guarantees the take_injection() drain below only runs when the send lands,
	// so injected motion / residual carry is never spent on a dropped report.
	if (inject.mouse_dirty && mouse_silent &&
	    cached_mouse_ep && mouse_layout.valid &&
	    usb_device_in_ep_free(cached_mouse_ep)) {
		uint8_t synth[64];
