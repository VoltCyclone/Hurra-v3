...
#  endif
	...
	for (uint8_t i = 0; i < desc.num_ifaces; i++) {
		...
		// bInterval -> poll period in µs. The captured device enumerates
		// full-speed, where bInterval is in 1 ms frames. Clamp to >=1 ms so a
		// garbage bInterval can't busy-poll, and poll a touch early (interval -
		// slack) to catch the fresh report without flooding NAKs.
		{
	...
	while (1) {
		...
		// silent (the merge path above only fires on real reports). A one-per-ms cap
		// and the merged_this_cycle gate keep the synth and merge paths from both
		// emitting in one frame.
