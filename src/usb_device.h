...
// True iff a send on this IN endpoint would be accepted right now: the clone is
// configured, ep_num is an armed IN endpoint, and its previous report has been
// consumed by the host (TX back to NAK). Used to ACK-pace synth injection to the
// host's real poll rate instead of a fixed millisecond cap.
