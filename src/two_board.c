...
#include "usb_host.h"
#include "usb_desc_interval.h"   // bInterval_to_us (speed-aware poll period)
...
void two_board_host_run(void)
{
    ...
    for (uint8_t i = 0; i < desc.num_ifaces; i++) {
        ...
        // bInterval -> poll period in µs, decoded against the captured device's
        // speed (HS bInterval is an exponent, not milliseconds). desc.speed was
        // set from usb_host_device_speed() above. See usb_desc_interval.h.
        uint32_t us = bInterval_to_us(desc.ifaces[i].interrupt_in_interval,
                                      desc.speed);
        if (us > 300u) us -= 250u;         // small lead to avoid polling late;
                                           // sub-300µs HS periods (b=1,2) poll on
                                           // schedule (no lead) — intentional.
