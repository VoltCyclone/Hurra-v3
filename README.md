...
# Flash a single role, reusing the existing build:
...
**Raw-input classifier** — the learned (2D-CNN) counterpart to the analyzer: trains a
PyTorch classifier to separate human vs. firmware-driven input and reports AUC/EER +
per-channel ablation. AUC→0.5 means the firmware wins. Standalone; not in the default
build. See `tools/raw_input_classifier/README.md`.
